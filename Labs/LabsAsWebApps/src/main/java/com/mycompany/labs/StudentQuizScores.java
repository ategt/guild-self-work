...
public class StudentQuizScores implements Game {
    ...
    public String readTextFile() {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                String scoresText = dataString[2];

                List<Integer> scoresList = new ArrayList<>();
                String[] scoresArray = scoresText.replace("[", "").replace("]", "").split(",");

                for (String scoreText : scoresArray) {
                    scoresList.add(Integer.parseInt(scoreText));
                }

                Student student = new Student(scoresList, id, name);
    ...
    public void putStudentInList(Student student) {

        int studentId = student.getStudentId();
        if (studentIdList.size() < studentId) {
            expandTheDataList(studentId);
        }
        studentIdList.add(studentId, student);

    }

    public void expandTheDataList(int studentId) {
        while (studentIdList.size() < studentId) {
